...
#include "globals.h"
#include "sha1.h"
static void sha1_compress_software(uint32_t state[5], const uint8_t* data);

static void sha1_compress_cpu(uint32_t digest[5], const uint8_t* block);
...
static void sha1_compress_cpu(uint32_t digest[5], const uint8_t* block)
{
    sha1_compress_software(digest, block);
}
#endif
